...
import com.kenneth.kindergarten.entities.Shelf;
...
public class KindergartenBean {
	...
	public Kindergarten create() {
		...
		for(Child child : childrenOfEkan){
			...
			if(child.getShelf() != null){
				em.persist(child.getShelf());
			}
		}
	...
	private void addshelfToElin(Child childElin) {
		
		Shelf shelf = new Shelf();
		shelf.setPosition("Hallen plan 2");
		shelf.setChild(childElin);
		childElin.setShelf(shelf);		
	}
