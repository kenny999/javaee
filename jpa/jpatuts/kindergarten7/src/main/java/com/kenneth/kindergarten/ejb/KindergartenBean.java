...
public class KindergartenBean {
	...
	public void addAndRemoveChild(Long kindergartenId) {
		
		System.out.println("*** BEGIN addAndRemoveChild ***");
		
		// Add child Nilsbert, with Garments Spidermantröja
		// Associate Nilsbert with the first section
		// Associate Nilsbert with the first Educator of this section
		// Remove child Manda
		
		Kindergarten kindergarten = em.find(Kindergarten.class, kindergartenId);

		Child child = new Child();
		child.setName("Nilsbert");
		...
		query.setParameter("childName", "Manda");
		Child childManda = query.getSingleResult();
		em.remove(childManda);
		
		System.out.println("*** END addAndRemoveChild ***");
	}
	
	public void removeAndAddChild(Long kindergartenId) {
		
		System.out.println("*** BEGIN removeAndAddChild ***");
		
		// Remove child Elis
		// Add child Saralotta, with Garments Spidermantröja
		// Associate Saralotta with the first section
		// Associate Saralotta with the first Educator of this section

		TypedQuery<Child> query = em.createNamedQuery(Child.findChildByName, Child.class);
		query.setParameter("childName", "Elis");
		Child childElis = query.getSingleResult();
		em.remove(childElis);
		
		Kindergarten kindergarten = em.find(Kindergarten.class, kindergartenId);

		Child child = new Child();
		child.setName("Saralotta");
		...
		System.out.println("*** END removeAndAddChild ***");
	}
