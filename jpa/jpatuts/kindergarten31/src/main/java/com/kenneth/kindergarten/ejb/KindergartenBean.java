...
public class KindergartenBean {
	...
	public Child getChildWithEducatorAndChilds1(String name) {
		System.out.println("Enter getChildWithEducatorAndChilds1");
		
		TypedQuery<Child> query = em.createNamedQuery(Child.findChildByName, Child.class);
		query.setParameter("childName", name);
		EntityGraph<?> graph = em.getEntityGraph("Child.graph1");
		query.setHint("javax.persistence.fetchgraph", graph);
		Child child = query.getSingleResult();
		return child;
	}

	public Child getChildWithEducatorAndChilds2(String name) {
		System.out.println("Enter getChildWithEducatorAndChilds2");
	...
	public Child getChildWithEducatorAndChilds3(String name) {
		System.out.println("Enter getChildWithEducatorAndChilds3");
		
		/*
		 * 
		 * @NamedEntityGraph(name="Child.graph1",
attributeNodes={
@NamedAttributeNode(value="educator", subgraph="educatorgraph")},
subgraphs={
		@NamedSubgraph(name="educatorgraph",
				attributeNodes={
						@NamedAttributeNode("childs")}),
})
		...
		EntityGraph<?> graph2 = em.createEntityGraph(Child.class);
		graph2.addAttributeNodes("educator");
		graph2.addSubgraph("educator").addAttributeNodes("childs");
		
		query.setHint("javax.persistence.fetchgraph", graph2);
		Child child = query.getSingleResult();
		return child;
	}
}
