...
public class KindergartenBean {
	...
	private void setEnumsToChild(Child childElin) {
		childElin.setMoodType(MoodType.SWEET);
		childElin.setAllocationType(AllocationType.HALF_TIME);
	}
	...
	public void printEnums(String childName){
		TypedQuery<Child> query = em.createNamedQuery(Child.findChildByName, Child.class);
		query.setParameter("childName", childName);
		Child child = query.getSingleResult();
		System.out.println("MOODTYPE:"+child.getMoodType());
		System.out.println("ALLOCATIONTYPE:"+child.getAllocationType());		
	}
