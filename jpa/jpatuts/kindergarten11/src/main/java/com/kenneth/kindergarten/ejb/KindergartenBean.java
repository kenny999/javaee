...
import com.kenneth.kindergarten.entities.CuddlyToy;
...
public class KindergartenBean {
	...
	public Kindergarten create() {
		...
		for(Child child : childrenOfEkan){
			...
			List<CuddlyToy> cuddlyToys = child.getCuddlyToys();
			if(cuddlyToys != null){
				for(CuddlyToy cuddlyToy : cuddlyToys){
					em.persist(cuddlyToy);
				}
			}
		}
	...
	private void setCuddlyToysOfElin(Child childElin) {
		List<CuddlyToy> l = new ArrayList<>();
		
		l.add(oneCuddlyTo("yellow",  "Bamse", childElin));
		l.add(oneCuddlyTo("brown",  "Lisa", childElin));
		l.add(oneCuddlyTo("red",  "Anna", childElin));
		l.add(oneCuddlyTo("white",  "Snowwhite", childElin));
		l.add(oneCuddlyTo("black",  "Herr Nilsson", childElin));
		
		childElin.setCuddlyToys(l);
	}
	
	private CuddlyToy oneCuddlyTo(String color, String name, Child child) {
		CuddlyToy ct = new CuddlyToy();
		ct.setChild(child);
		ct.setColor(color);
		ct.setName(name);
		return ct;
	...
	public void getCuddlyToys() {
		TypedQuery<Child> query = em.createNamedQuery(Child.findChildByName, Child.class);
		query.setParameter("childName", "Elin");
		Child child = query.getSingleResult();
		if(child != null){
			for(CuddlyToy cuddlyToy : child.getCuddlyToys()){
				System.out.println(cuddlyToy.getName() + ","+cuddlyToy.getColor());
			}
		}
	}
}
