...
public class KindergartenBean {
	...
	public void bulkUpdateGarments(String newname) {
		
		Query q = em.createQuery("UPDATE Garment g set g.name = :newname");
		q.setParameter("newname", newname);
		q.executeUpdate();
	}

	public void buldDeleteEducators() {
		
		Query q0 = em.createQuery("UPDATE Child c set c.educator = null");
		q0.executeUpdate();
		
		Query q = em.createQuery("DELETE from Educator e");
		q.executeUpdate();
	}
